...
namespace
{
  std::wstring Utf8ToWide(const std::string &value)
  {
    ...
    MultiByteToWideChar(
        CP_UTF8, MB_ERR_INVALID_CHARS, value.data(), static_cast<int>(value.size()), output.data(), count);
    return output;
  }
  ...
  std::string WideToUtf8(const wchar_t *value)
  {
    ...
    if (count <= 0) throw std::runtime_error("Invalid UTF-16 string");
    std::string output(count, '\0');
    WideCharToMultiByte(
        CP_UTF8, WC_ERR_INVALID_CHARS, value, length, output.data(), count, nullptr, nullptr);
    return output;
  }

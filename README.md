...
![Windows 视频批量混淆](docs/screenshots/video-tools-desktop.png)
...
- **视频性能档位**：生成与还原分别保存“普通”“全功率”配置；全功率模式启用多核逐帧工作池、更大的流式块和更快的 FFmpeg 编码预设，普通模式限制核心数以兼顾前台使用。
- **视频批量与文件夹**：可多选视频、连续导入多个文件夹，队列默认折叠并保留原文件夹及子目录；压缩输出支持每个文件、每个文件夹或全部合并，并复用密码库。
- **视频双重还原**：原始混淆 MP4 内嵌流式原视频副本与 SHA-256，可无损精确还原；平台重新编码后可自动读取算法标识或手动选择算法与种子进行近似还原。
- **链接解析与账号会话**：视频页可粘贴支持的平台分享链接或视频直链；B站、抖音登录按钮在软件内打开官方登录页，登录 Cookie 经系统安全存储保存并只在对应站点解析时使用，软件不保存账号密码。
...
- **窗口尺寸记忆**：Windows 首次启动使用 1302×842；关闭时保存正常窗口的位置和尺寸，下次启动恢复，并校验窗口仍位于可见显示器范围。
...
- 1440×900 Windows、390×844 与 360×640 Android 工作台、视频页、趣味工具、压缩设置、密码库、分享导入、算法选择器和 TXT 工作区黄金图布局检查；另覆盖 Android 横屏与 1.3 倍文字。视频页额外覆盖性能档位切换、批量文件夹、压缩设置与登录入口。
